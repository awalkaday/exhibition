// simulation-2/src/gallery/GalleryView.tsx
//
// Adapted from KilledByAPixel/fxhashArchive's src/gallery/GalleryView.tsx (MIT
// licensed — see simulation-2/THIRD_PARTY_LICENSES/fxhashArchive-LICENSE).
// The only change: the `Viewer` overlay is gone. In the source, walking up to a
// piece opens a full-screen runner that re-executes its generative code — there's
// no equivalent for a photograph, so `mode === 'view'` now just means the camera
// has arrived and locked, with nothing extra drawn. That's also the seam where the
// naming plaque ("who named this") will mount, once it exists.
...
export default function GalleryView() {
  ...
  return (
    <div className="gallery">
      ...
      {/* No Viewer here — see file header. `view` is still tracked (and still
          cleared correctly via onMode) for whatever mounts here next: the naming
          plaque, once a photo selection and the naming UI exist. */}
    </div>
  )
}
